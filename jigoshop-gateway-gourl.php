...
function gourl_jigoshop_gateway_load() 
{
	...
	*/
	...
	class Jigoshop_Gateway_GoUrl extends jigoshop_payment_gateway 
	{
	    ...
	    */
	    protected function get_default_options() 
	    {
	    	...
	    	$method_description  	.= sprintf(__( 'Accept %s payments online in Jigoshop.', GOURLJI ), ($this->coin_names?ucwords(implode(", ", $this->coin_names)):"Bitcoin, Litecoin, Paycoin, Dogecoin, Dash, Speedcoin, Reddcoin, Potcoin, Feathercoin, Vertcoin, Vericoin, Peercoin")).'<br/>';
	    	...
	    	$defaults[] = array(
	    			'id' 		=> GOURLJI.'enabled',
	    			'name'		=> __('Enable GoUrl Bitcoin/Altcoin Payments', GOURLJI),
	    			'desc' 		=> sprintf(__( 'Enable Bitcoin/Altcoin Payments in Jigoshop with <a href="%s">GoUrl Bitcoin Gateway</a>', GOURLJI ), $url3),
	    			'tip' 		=> '',
	    			'std' 		=> 'no',
	    			'type' 		=> 'checkbox',
	    			'choices'	=> array(
    ...
    /*
     * 5.4 Output for the order received page.
     */
    public function cryptocoin_payment( $order_id )
	{
		...
		elseif (!$this->payments || !$this->defcoin || true === version_compare(JIGOSHOP_VERSION, '1.12', '<') || true === version_compare(GOURL_VERSION, '1.3', '<') || 
		...
		{ 	
			...
			{
				echo '<h2>' . __( 'Information', GOURLJI ) . '</h2>' . PHP_EOL;
				echo "<div class='error'>". sprintf(__( 'This order&rsquo;s amount is &ldquo;%s&rdquo; &mdash; it cannot be paid for. Please contact us if you need assistance.', GOURLJI ), $amount ." " . $currency)."</div>";
			}
			else
			{
				...
				if ($currency != "USD" && !$crypto)
				{
					...
					{
						echo '<h2>' . __( 'Information', GOURLJI ) . '</h2>' . PHP_EOL;
						echo "<div class='error'>".sprintf(__( 'Sorry, but there was an error processing your order. Please try later or use a different payment method. Cannot receive exchange rates for %s/USD from Google Finance', GOURLJI ), $currency)."</div>";
					}
					else $currency = "USD";
				}
				...
				if ($amount > 0)
				{
					...
					else
					{
						...
						if ($result["is_paid"])
						{ 
							echo "<div align='center'>" . sprintf( __('%s Payment ID: #%s', GOURLJI), ucfirst($result["coinname"]), $result["paymentID"]) . "</div><br>";
						}
					}
				}	
			}
	    ...
	    /*
	     * 5.6 GoUrl Bitcoin Gateway - Instant Payment Notification
	     */
